...
@section('content')
                ...
                <div class="achv-card">
                    <div class="achv-icon" style="background: {{ $color }}">
                        ...
                        <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                            <path
                                d="M17 3H7v4a5 5 0 1 0 10 0V3Zm-5 13a7 7 0 0 1-7-7V3H3v2a7 7 0 0 0 6 6.93V21l3-1 3 1v-9.07A7 7 0 0 0 21 5V3h-2v6a7 7 0 0 1-7 7Z" />
                        </svg>
                    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            @if (session('earned_badge') || session('achievement_earned'))
                if (typeof playLevelWinSound === 'function') {
                    playLevelWinSound();
                } else {
                    // Fallback: play a local audio once if the helper isn't available
                    const s = new Audio("{{ asset('audio/badge-unlocked.mp3') }}");
                    s.currentTime = 0;
                    s.play().catch(() => {});
                }
            @endif
